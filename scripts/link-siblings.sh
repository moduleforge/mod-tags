...
# sibling-relative paths (Go module replace directives) resolve correctly
# no matter how deeply a Flow task or plan worktree nests this checkout
# below the moduleforge aggregate root.
#
# Canonical explanation of the mechanism, why it exists, and the full
# wiring convention (make preflight as a prerequisite of build/test):
# docs/mf-standards/building-common.md#building-inside-a-task-worktree
#
# SIBLINGS below is (mod-core) only: api/go.mod's other two replaces
# (tags-model => ../model) are intra-repo and need no symlink.
#
# Env:
#   MODULEFORGE_SIBLINGS_DIR — override the aggregate directory that holds
#     mod-core/. Defaults to the parent directory of this repo's main
#     checkout.
